...
**Файлы**:
- `data/bank_accounts.json` — счета и карты: номер карты (16 цифр), хэш PIN, баланс, флаг блокировки, срок действия карты (expiry_date). При первом запуске создаётся файл с демо-счетами. Если остался старый файл с короткими номерами карт — удалите его, чтобы создались новые 16-значные счета.
- `data/atm_state.json` — состояние кассет банкомата. Обновляется при выдаче/приёме наличных, пополнении и изъятии инкассатором.

**Блокировка карт**: при трёх неверных вводах PIN карта блокируется в банке (`is_blocked: true` в `bank_accounts.json`) и изымается банкоматом. Заблокированная карта при вставке не даёт вводить PIN — банкомат сразу изымает её.
