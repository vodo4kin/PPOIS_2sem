...
**Язык написания кода**: Python 3.10+

**Диаграммы**: PlantUML

## Текстовое описание предметной области

**Банкомат (ATM — Automated Teller Machine)** — автоматизированное устройство самообслуживания для взаимодействия клиента с банковским терминалом: выполнение операций с использованием банковской карты, пин-кода, счёта и наличных денег при связи с банком.

### Основные операции

- Операция ввода пин-кода (аутентификация по карте и PIN)
- Операция снятия/внесения наличных
- Операция проверки баланса счёта
- Операция перевода средств между счетами
- Операция выдачи чека (после выполнения операции — диалог «Print receipt? (y/n)», при выборе «y» чек печатается на экран)  
...
3. **Завершение сессии** одним из следующих способов:
   ...
   - **изъятие карты** банкоматом с одновременной блокировкой в системе банка (при многократном неверном вводе PIN)
...
- **Инкассатор** (Cash Replenisher)  
  Сотрудник банка или сервисной организации. Выполняет обслуживание банкомата: пополнение кассет наличными, изъятие наличных, замена кассеты. Имеет доступ по служебной карте + PIN.
...
- **Аутентификация (операция ввода пин-кода)** — Клиент вставляет карту и вводит PIN-код. Банк проверяет данные и разрешает/запрещает доступ.  
...
- **Просмотр баланса (операция проверки баланса)** — После успешной аутентификации клиент получает информацию о доступном остатке на счёте.
...
- **Выдача чека** — После каждой из операций (баланс, снятие, внесение, перевод, оплата, смена PIN) предлагается печать чека: диалог «Print receipt? (y/n)»; при выборе «y» чек выводится на экран (дата/время, операция, результат).
...
<div align="center">
    <img src="ATM/uml/img/ATM_Service_StateMachine.svg" width="600" alt="Диаграмма состояний инкассатора">
</div>

### Сессия технического специалиста

<div align="center">
    <img src="ATM/uml/img/ATM_Technician_StateMachine.svg" width="600" alt="Диаграмма состояний технического специалиста">
</div>
...
- **UserInterfaceModule** — отвечает за взаимодействие с пользователем через CLI  
  ...
  - `ReceiptPrinter` — печать чека после операции (диалог «Print receipt? (y/n)», вывод чека на экран)
...
- **Сквозные классы**  
  ...
  - `Config` — глобальные настройки. Сохранение состояния выполняют `StateSaver` (кассеты) и `MockBankRepository` (счета)
...
## Известные недочёты (не исправлены)

В рамках лабораторной работы следующие ограничения **не реализованы** и остаются как возможные улучшения:

1. **Пополнение кассет без лимита**  
   Инкассатор и клиент может указывать при пополнении сколь угодно большие суммы (миллиарды, триллионы и т.д.). Ограничение на максимальный объём пополнения за одну операцию или на суммарную вместимость кассет не проверяется.

2. **Роль карты при входе инкассатора/техника**  
   Авторизация в режиме инкассатора или техника выполняется через общий сервис аутентификации по любой карте из базы, для которой верен введённый PIN. Отдельной проверки «это служебная карта инкассатора/техника» (например, по списку разрешённых номеров или роли в БД) нет — войти можно любой картой, имеющейся в БД.

3. **UML диаграмма**
    В диаграмме классов отсутствуют атрибуты и методы.
